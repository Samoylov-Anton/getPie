    ...
    $.get("/getShowCaseList/" + cityId, function (x) {
        ...
        for (var i in x) {
            ...
            for (var a in x[i].imageList) {
                t += '<div class= "col-xs-5 col-sm-3" style="height: 120px; width: 120px; padding: 0px; margin-right: 10px;margin-left: 10px;"><a rel = "lightbox" href = "' + x[i].imageList[a].image + '">' +
                    '<img src = "' + x[i].imageList[a].image + '" class = "img-rounded" style = "width: 100%; height: 100%; object-fit: cover;">' +
                    '</a > </div >';
            }
